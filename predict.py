...
def main(img_pattern: str = None,
    ...
    if img_pattern is None:
        video = True
        img_pattern = './datasets/my_dataset/qwatch/1280_2.mp4'

    def sorted_glob(pattern):
        return sorted(glob(pattern))
    print('check1:', img_pattern)
...
weights_path = 'fpn_inception.h5'
predictor_one = Predictor(weights_path=weights_path)
def pred_one_image(img=None):
    if img is None:
        img_name = './datasets/my_dataset/qwatch/fast.jpg'
    ...
    pred = predictor_one(img, mask)
    # cv2.imshow('result1', pred)
    # cv2.waitKey(0)

    # ぶれ除去の二回目を行う
    # pred = predictor_one(pred, mask)
    # cv2.imshow('result2', pred)
    # cv2.waitKey(0)
...
if __name__ == '__main__':
    ...
    while True:
        num = i % 3
        print(num)
        name_list = ['./datasets/my_dataset/qwatch/fast.jpg',
                     './datasets/my_dataset/qwatch/middle.jpg',
                     './datasets/my_dataset/qwatch/slow.jpg']
        img = cv2.imread(name_list[num])
        pred_one_image(img=img)
        i += 1
